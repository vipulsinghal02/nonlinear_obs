...
% \title{A primer on nonlinear observability and identifiability with applications in synthetic biology} % for perturbational control of transcriptional states
% % ---\title{Controllability and Observability of Transcriptomic States}
...
% \email{vipul@integratedbiosciences.com}
% \begin{document}
...
\usepackage{mathrsfs,mathtools,dsfont}
\usepackage{xcolor}
\usepackage{hyperref}
\usepackage{todonotes}  % For development notes
...
\geometry{
  ...
  top=1in,
  bottom=1in,
  left=1in,    % Add this line
  right=1in,   % Add this line
  heightrounded
}
...
\newcommand{\fracpartial}[2]{\frac{\partial #1}{\partial #2}}
\newcommand{\isdefined}{\triangleq}
\newcommand{\liebrack}[2]{[#1,#2]}  % Lie bracket notation
\DeclareMathOperator{\ann}{ann}      % Annihilator
% \DeclareMathOperator{\ker}{ker}      % Kernel

\begin{document}

\title{A Primer on Nonlinear Control\\
with Applications in Synthetic Biology}
\author{Vipul Singhal}
\address{Integrated Biosciences, 900 Island Drive, Redwood City, CA 94065}
\email{vipul@integratedbiosciences.com}

\begin{abstract}
We present a brief overview of nonlinear observability, controllability and identifiability, and outline applications in systems and synthetic biology. We begin with some foundational ideas from geometry, namely manifolds, vector and covector fields, Lie derivatives and brackets, and Frobenius' theorem), followed by key tests of controllability and observability, and finally describe minimal realizations of nonlinear systems. This leads to practical methods for identifying unobservable or unreachable states, along with non-identifiable parameters in nonlinear systems. [add more about controlling and observing states]
\end{abstract}

\maketitle

\section{Introduction}
% Differential geometry provides the mathematical foundation for understanding and controlling nonlinear systems, particularly in modern biology. 

% Consider a genetic regulatory network where:
% \begin{itemize}
% \item Genes interact through complex feedback loops
% \item Proteins modify expression through nonlinear activation/repression
% \item Epigenetic states evolve through intricate biochemical mechanisms
% \end{itemize}

% Unlike linear systems where effects are proportional to inputs, biological networks exhibit rich behaviors including:
% \begin{itemize}
% \item Bistability in gene expression
% \item Oscillatory dynamics in protein levels
% \item Threshold effects in cellular responses
% \end{itemize}

% % previous stuff: 

% The focus of these notes is on biology, where these tools offer powerful frameworks for understanding and controlling cellular and genetic systems. They might be used to design control strategies that steer cellular states through precise combinations of drug dosing and optogenetic interventions. Additionally, they enable the analysis of the observability of intricate genetic networks, especially when limited measurement data---such as fluorescent reporters tracking single-gene dynamics or RNA sequencing capturing genome-wide snapshots at single time points---is available. These tools also provide methods to evaluate parameter identifiability in nonlinear dynamical models, ensuring that model predictions align with biological reality. Moreover, they facilitate a deeper understanding of the geometry of state and parameter spaces, revealing the intricate structure and behavior of biological systems.

% In biology, these tools could be used to:
% \begin{itemize}
% \item Design control strategies for steering cellular states using combinations of drug dosing and optogenetic interventions
% \item Analyze the observability of complex genetic networks from limited measurements (e.g., RNA sequencing data)
% \item Evaluate parameter identifiability in nonlinear dynamical models of cellular processes
% \item Understand the geometry of state space and parameter space in biological systems
% \end{itemize}

% Two fundamental concepts that we'll explore in depth are nonlinear observability and controllability. Observability addresses whether we can determine a system's full state (e.g., protein concentrations, epigenetic modifications) from partial measurements (e.g., RNA sequencing and/or fluorescent reporters). Controllability examines whether we can drive the system to desired states through available interventions. In the biological context, this might involve using drug combinations to steer transcriptomic profiles, applying optogenetic perturbations to control protein activity, or modifying epigenetic states through targeted interventions. These properties, while straightforward for linear systems, require sophisticated geometric tools when applied to the nonlinear dynamics typical of biological networks. A related concept, parameter identifiability, helps us understand which kinetic parameters in our models can be determined from experimental data---crucial for building accurate representations of cellular processes.


% # ----------------------------- chat gpt --------------------------------- #
% Differential geometry provides the mathematical foundation for understanding and controlling nonlinear systems, a crucial capability in modern engineering and science. While linear approximations offer valuable insights, many systems in engineering and science---from spacecraft attitude control to robot manipulation to biological cell regulation---are fundamentally nonlinear. This nonlinearity necessitates sophisticated mathematical tools for analysis and control.

% Consider a genetic regulatory network: genes interact through complex feedback loops, proteins modify expression levels through nonlinear activation and repression functions, and epigenetic states evolve through intricate biochemical mechanisms. Unlike linear systems where effects are proportional to inputs, biological networks exhibit rich behaviors like bistability, oscillations, and threshold effects. This nonlinearity makes both control and state estimation particularly challenging, yet these fields are active areas of research with considerable potential for advancing therapeutic strategies.

% The mathematical tools we will develop in this text---smooth manifolds, tangent spaces, vector fields, and more---provide the language to describe nonlinear systems and design controllers for them. These tools have enabled breakthroughs in diverse fields. In aeronautics, they underpin the precision control of aircraft and spacecraft orientation. In robotics, they facilitate motion planning for robotic arms and autonomous vehicles, and in machine learning, they provide insights into the geometry of optimization landscapes, enhancing our understanding of how algorithms converge.

% The focus of these notes is on how differential geometric ideas might be used for understanding and controlling genetic systems. They can be used to design control strategies for steering cellular states through precise combinations of drug dosing and optogenetic interventions. They might also be used for analyzing the observability of transcriptomic or proteomic states, even with limited measurement data, such as fluorescent reporters tracking single-gene dynamics or RNA sequencing capturing genome-wide snapshots at single time points. These tools also help evaluate parameter identifiability, ensuring that model predictions align with experimental data. Furthermore, they facilitate a deeper understanding of the geometry of state and parameter spaces, shedding light on the intricate structures and dynamics underlying biological systems and paving the way for innovative approaches to modeling and intervention.

% Two fundamental concepts we’ll explore in depth are nonlinear observability and controllability. Observability addresses whether we can infer a system’s full state---such as protein concentrations or epigenetic modifications---from partial measurements, like RNA sequencing or fluorescent reporters. Controllability, on the other hand, examines whether we can drive the system to desired states using available interventions. In biology, this might involve steering transcriptomic profiles with drug combinations, controlling protein activity through optogenetic perturbations, or modifying epigenetic states with targeted interventions. While these properties are relatively straightforward to analyze in linear systems, the nonlinear dynamics typical of biological networks demand sophisticated geometric tools to capture their complexity.

% A closely related concept, parameter identifiability, focuses on determining which kinetic parameters in a model can be inferred from experimental data---a critical step in building accurate representations of cellular processes. Together, these concepts form the foundation for understanding and controlling the intricate behavior of biological systems.

% This text begins with fundamental concepts from differential geometry and progresses toward their applications in biological control theory. The following sections review essential topics, including smooth manifolds, coordinate charts, tangent spaces, and flows generated by vector fields. These concepts form the foundation for analyzing key properties such as controllability, observability, and identifiability in biological systems.

% For readers interested in further exploration, \cite{nijmeijer_nonlinear_1990} offer an excellent introduction to control applications, while \cite{boothby_2002} provides a thorough grounding in geometric foundations. For a more advanced perspective, \cite{abraham_manifolds_1988} deliver an in-depth treatment of these ideas. Ultimately, this text aims to equip readers with both the theoretical understanding and practical tools to analyze and control complex biological systems.

% # ----------------------------- Gemini --------------------------------- #

Differential geometry provides the mathematical foundation for understanding and controlling nonlinear systems, a crucial capability in modern engineering and science. While linear approximations offer valuable insights, many systems in engineering and science, including spacecraft attitude control, robot manipulation, and biological systems, exhibit inherently nonlinear behavior. Consider the intricate dynamics of a gene regulatory network: genes interact through complex feedback loops, proteins modify expression levels through nonlinear activation and repression functions, and epigenetic states evolve through intricate biochemical mechanisms. Unlike linear systems, where effects are proportional to inputs, these networks exhibit rich behaviors like bistability, oscillations, and threshold effects, making both control and state estimation particularly challenging.

The mathematical tools we will develop in this text – smooth manifolds, tangent spaces, vector fields, and more – provide the language to describe and analyze these nonlinear systems. These tools have enabled breakthroughs in diverse fields, including aeronautics, robotics, and machine learning. In aeronautics, they underpin the precision control of aircraft and spacecraft orientation. In robotics, they facilitate motion planning for robotic arms and autonomous vehicles. In machine learning, they provide insights into the geometry of optimization landscapes, enhancing our understanding of how algorithms converge.

This text focuses on applying these powerful tools to understand and control genetic systems. For example, we can use differential geometry to design control strategies for steering cellular states through precise combinations of drug dosing and optogenetic interventions. Imagine controlling the expression of a specific gene involved in cancer progression by manipulating the activity of key regulatory proteins.

Two fundamental concepts we'll explore in depth are nonlinear observability and controllability. Observability addresses whether we can infer a system's full state – such as protein concentrations or epigenetic modifications – from partial measurements, like RNA sequencing or fluorescent reporter data. Controllability examines whether we can drive the system to desired states using available interventions. In the context of our example, this might involve determining whether we can effectively control the expression of the cancer-related gene by manipulating the activity of its regulators.

A closely related concept, parameter identifiability, focuses on determining which kinetic parameters in a model can be inferred from experimental data – a critical step in building accurate representations of cellular processes. These concepts form the foundation for understanding and controlling the intricate behavior of biological systems.

This text begins with fundamental concepts from differential geometry, including smooth manifolds, coordinate charts, tangent spaces, and flows generated by vector fields. These concepts form the foundation for analyzing key properties such as controllability, observability, and identifiability in biological systems. For a deeper dive into these concepts, readers are encouraged to consult \cite{nijmeijer_nonlinear_1990}  for an excellent introduction to control applications and \cite{boothby_2002} for a thorough grounding in geometric foundations. For a morine advanced perspective, \cite{abraham_manifolds_1988} deliver an in-depth treatment of these ideas. 

This text aims to equip readers with the theoretical understanding and practical tools to analyze and control complex biological systems, ultimately paving the way for advancements in areas such as personalized medicine and synthetic biology.




% This text starts with a description of basic differential geometric concepts and progressing to their applications in biological control theory. While we'll maintain mathematical rigor, our focus will be on developing intuition and highlighting applications to real biological systems. In the following sections, we'll review essential concepts from differential geometry: smooth manifolds, coordinate charts, tangent spaces, and flows generated by vector fields. These provide the foundation for analyzing controllability, observability, and identifiability in biological systems. Readers seeking additional depth may consult standard references such as Nijmeijer \& van der Schaft (1990) for control applications or Boothby (2002) for geometric foundations. Abraham \& Marsden (1978) provides an advanced treatment suitable for deeper study. and Ratiu, Tudor

% Introduction to Differential Geometry and Nonlinear Control
...
We adopt the Einstein summation convention throughout: repeated indices (once as superscript, once as subscript) imply summation over their range. For example, given a vector $v$ and basis vectors $\{E_1,\ldots,E_n\}$, we have
\begin{equation}\label{eq:esc}
v = \alpha^i E_i \triangleq \sum_{i=1}^n \alpha^i E_i
\end{equation}
% Key conventions:
% \begin{itemize}
% \item Vectors have subscript indices ($E_i$)
% \item Coefficients use superscript indices ($\alpha^i$)
% \item Covectors use superscript indices with subscripts in coefficients, $\omega  = w_i dx^i$
% \item All vector spaces are real and finite dimensional
% \end{itemize}
% Furthermore, the index $i$ in $\frac{\partial}{\partial x^i}$ will be interpreted as a subscript; indeed, we will regularly abbreviate $\frac{\partial}{\partial x^i}\bigr|_a$ to $\partial_{ia}$, and later see that this is an element of $T_a\R^n$, the tangent space to $\R^n$ at a point $a$.
...
% \end{equation}
% 
All vector spaces will be real and finite-dimensional, and vectors and their coefficients will be indexed with subscripts and superscripts, respectively (as in Eq \eqref{eq:esc}). Similarly, covectors will have superscripts, while the corresponding coefficients will have subscripts, $\omega  = \sum_{i = 1}^n w_i dx^i = w_i dx^i$. Furthermore, the index $i$ in $\frac{\partial}{\partial x^i}$ will be interpreted as a subscript; indeed, we will regularly abbreviate $\frac{\partial}{\partial x^i}\bigr|_a$ to $\partial_{ia}$, and later see that this is an element of $T_a\R^n$, the tangent space to $\R^n$ at a point $a$. Note also that all vector spaces in this document will be finite dimensional. 

\subsection{Manifolds, charts and local representatives}

% {\color{red}
% \begin{defn}[Smooth Manifold]\label{def:smooth_manifold}
% An $n$-dimensional smooth manifold $N$ is a set with two key properties:
% \begin{enumerate}
%     \item It is locally Euclidean: each point has a neighborhood that can be mapped to an open subset of $\R^n$
%     \item It has a smooth structure: the transitions between different local descriptions are smooth
% \end{enumerate}
% \end{defn}

% To make this precise, we introduce the notion of coordinate charts. A coordinate chart is a pair $(U, \varphi)$ where:
% \begin{itemize}
%     \item $U \subset N$ is an open set in the manifold
%     \item $\varphi: U \to \R^n$ is a homeomorphism onto its image
% \end{itemize}

% The crucial aspect of a smooth manifold is how these charts relate to each other. Given two charts $(U, \varphi)$ and $(V, \phi)$ with overlapping domains ($U \cap V \neq \emptyset$), we require that:
% \begin{equation}\label{eq:transition_map}
%     \phi \circ \varphi^{-1}: \varphi(U \cap V) \to \phi(U \cap V)
% \end{equation}
% is a smooth diffeomorphism. This map is called a transition map or coordinate transformation.

% % notability a fig? or use the procreate app on ipad. 
% % \begin{figure}[h]
% %     \centering
% %     \includegraphics[width=0.8\textwidth]{manifold_charts.pdf}
% %     \caption{Visualization of coordinate charts and transition maps on a smooth manifold. Two overlapping charts $(U,\varphi)$ and $(V,\phi)$ provide local coordinates, with the transition map $\phi \circ \varphi^{-1}$ ensuring smooth compatibility between them.}
% %     \label{fig:manifold_charts}
% % \end{figure}


% \begin{example}[Gene Expression Space]\label{ex:gene_expression}
% Consider the space of possible expression levels for $n$ genes. While each gene's expression level is technically bounded, we can often model this locally as $\R^n_+$ (the positive orthant). Different experimental techniques (RNA-seq, microarrays, qPCR) provide different ``charts'' for measuring these expression levels, with transformation functions relating their measurements.
% \end{example}


% }

A smooth manifold is a set which is locally Euclidean---in the sense that neighborhoods of points can be modeled using Euclidean spaces---along with a certain \textit{differentiable structure}, which we describe below.
% \footnote{Our description will omit certain technical details, like the underlying topological space needing to have a second countable basis of open sets, and being Hausdorff. While important in their own right, these details do not usually make an appearance in proofs, and can safely be skipped in a first reading of nonlinear control. The interested reader can find them in any differential geometry text, such as \cite{abraham_manifolds_1988}.} 
We use the symbol $N$ to denote an $n$-dimensional smooth ($C^{\infty}$) manifold, and the tuples $(U, \varphi)$ and $(V, \phi)$ to denote coordinate charts, where $U$ and $V$ are open sets in $N$ and the homeomorphisms $\varphi$ and $\phi$ map them (respectively) onto open sets in $\R^n$. The key component of $N$'s differentiable structure is the following compatibility requirement for coordinate changes: whenever $U \cap V$ has a nonempty interior, the map $\phi\circ\varphi^{-1}: \varphi(U\cap V) \rightarrow \phi(U\cap V)$ is a (smooth) diffeomorphism. %This requirement is the critical aspect of what is known as the differentiable structure of a manifold, and turns the topological manifold into a \textit{smooth manifold}. 
% (which is used to define the differentiable structure of $N$, turning the topological manifold into a \textit{smooth manifold})

\begin{remark}
For simplicity, we omit certain technical requirements on the topology of $N$, such as the existence of a second countable basis of open sets and the Hausdorff property.\footnote{These properties ensure that the manifold is well-behaved and has a countable atlas of charts. While important for a complete understanding, they can be safely assumed for most applications in nonlinear control. For a thorough treatment, see Chapter 1 of \cite{abraham_manifolds_1988}.}
\end{remark}

A point $p\in U$ in local coordinates will typically be denoted by $a = \varphi (p) = (x^1(p), \dots, x^n(p)) \in \R^n$, where the $x^i: U \rightarrow \R$ are the ($C^{\infty}$) \textit{coordinate functions} corresponding to a given $\varphi$. Coordinate neighborhoods may therefore also be denoted by $(U, x^1, \dots x^n)$. 
...
The space of \textit{derivations}, denoted $\mathscr{D}(p)$, is the vector space of mappings $C^{\infty}(p)\rightarrow \R$ such that for all $\alpha, \beta \in \R$ and $f, g \in C^{\infty}(p)$ the following two properties,
...
\end{aligned}
\end{equation}
hold, with vector space operations, 
\begin{equation}
\begin{aligned}
...
(\alpha X_p)(f) &={}\alpha X_p (f), \qquad \alpha \in \R.
 ...
 \end{equation}
\end{defn}
It may be verified that directional derivatives satisfy these properties, and are therefore derivations (and in fact, on the algebra of smooth functions $C^{\infty}(p)$, these are known to be the \textit{only} type of derivations to exist.)\footnote{which is not the case on $C^r(p)$, for $r<\infty$} These ideas are tied together by the following theorem. 
\begin{thm}
...
Using this isomorphism, we can {identify} $T_p\R^n$ with $\mathscr{D}(p)$. We write vectors as linear combinations of basis vectors, $X_p = \alpha^i \partial_{ip}$, so that the $E_{ip}$ are identified with $\partial_{ip}$, and $E_{ip}f = \partial_{ip} f$. Note that $X_p x^i = \left(\alpha^j \partial_{jp}\right) x^i = \alpha^j \delta^i_j = \alpha^i$, because $\frac{\partial x^i}{\partial x^j} = \delta^i_j = 1$ if $i=j$ and $0$ otherwise ($\delta^i_j$ is called the Kronecker delta). Consequently, $X_p$ is completely determined by its action on the coordinate functions, $x^i$. This equivalence between derivations on functions and the tangent space will be especially useful in defining the tangent space to manifolds, a topic we cover next. 
...
\end{aligned}
\end{equation}
One also defines the natural projection $\pi : TN \twoheadrightarrow N : (p, v) \mapsto p$. %, which, in our case, looks like $(p, X_p) \mapsto \pi((p, X_p)) = p$. 
Note that some authors define the tangent bundle without the base point explictly included, $TN = \cup_{p\in N}T_pN$, with projection $v \mapsto \pi(v) = p$ for $v \in T_pN$. 

In some cases, it is possible to find an isomorphism between the tangent bundle and the Cartesian product of the base space and some vector space, as in the cases of $T\R^n \cong \R^n \times \R^n$ or $T\mathbb{S}^1 \cong \mathbb{S}^1 \times \R$. In this case, the tangent bundle is called the \textit{trivial bundle}. In general, however, such a decomposition will not exist (the most common example being $T\mathbb{S}^2$), and we will always use the definition in equation \ref{eq:tangentbundle}. The tangent bundle plays an instrumental role in the definition of (smooth) vector fields on manifolds (Section \ref{sec:vf_flows_lie} below). 

\subsubsection{Pushforward (differential) of a map}
To describe elements of $T_pN$ in local coordinates, we require two definitions.
...
A corollary of the last statement in Definition \ref{def:pushforward} is that pushforwards created out of diffeomorphisms are isomorphisms (consider, for instance, the identity isomorphism $F_*^{-1}\circ F_*$ constructed out of the identity map $F^{-1}\circ F$). An important implication of this is that, for a chart $(U\ni p,\, \varphi)$, the spaces $T_pU$ and $T_{a}\left(\varphi(U)\right) \cong T_{a}\R^n$ are isomorphic, with $a = \varphi(p)$. The basis $ \left\{\partial_{ia}\right\}_{i = 1}^n$ of $T_{a}\R^n$ defines a basis of $T_pU$, 
\begin{equation}
\begin{aligned}
\varphi^{-1}_*&:T_{a}\R^n\rightarrow T_pU, \qquad a = \varphi(p),\\
E_{ip} &\triangleq{} \varphi^{-1}_*\partial_{ia}, \qquad i = 1, \dots, n.
\end{aligned}
\end{equation}


This basis is called a {coordinate frame} at $p\in U$, and the {coordinate frames on $U$} comprise the collection of such bases on all points in $U$, that is, $\left\{ \{E_{ip}\}_{i=1}^n \; \bigr| \;p\in U \right\} \subset TU$. When referring to the frames on $U$ as a whole, or to an arbitrary frame (with base point $p$ unspecified), we will use the symbols $E_1, \dots, E_n$, or, in local coordinates, $\partial_1, \dots, \partial_n$. Notice that unlike in the $T_p\R^n$ case discussed previously, $\{E_{i}\}_{i = 1}^n$ and $\{\partial_{i}\}_{i = 1}^n$ cannot be {identified} with each other, although are equivalent up to isomorphism (via $\varphi_*$), \textit{which reduces the study of $T_pU$ to that of $T_{a}\R^n$}. 

Given $f\in C^{\infty}(p)$ with local representative $\hat f = f\circ \varphi^{-1}$, we have $E_{ip}f = \partial_{ia}\hat f$. In particular, for coordinate functions $x^j$, we have $E_i x^j = \partial_i \hat x^j = \delta_{i}^j$. A vector may then be written $X_p = \alpha^i E_{ip}$, with $\alpha^i = X_p x^i$. 
...
Let $(U, \varphi) = (U, x^1, \dots, x^n)$ and $(V, \phi) = (V, y^1, \dots, y^m)$ be charts on $N$ and $M$ respectively, with $F(U)\subset V$, $a = \varphi(p) = (x^1, \dots, x^n)$. Let the local representative of $F(p)$ be $f(a) = \phi \circ F \circ \varphi^{-1}(x^1, \dots, x^n)$, with components given by $f^j(x^1, \dots, x^n) = y^j(F\circ \varphi^{-1}(x^1, \dots, x^n))$. Some authors use the symbols $f^j$ and $y^j$ interchangeably, but note that their domains are different---$\varphi(U)$ and $V$ respectively---and we will treat them as distinct functions. 
% Fortunately we will not be sloppy in this way; this will come at the cost of slightly increased verbosity. 
Furthermore, in this section we will distinguish the local representatives of the bases of $T_pN$ and $T_{F(p)}M$ by writing them out in full: $\frac{\partial}{\partial x^i}$ and $\frac{\partial}{\partial y^j}$. 
...
\begin{align}\label{eq:pushforward_in_lr_coeff}
...
In matrix termis, Eq. \ref{eq:pushforward_in_lr_coeff} can be written 
\begin{align}\label{eq:pushforward_in_lr_matrix}
\left[ \begin{array}{c}
{\beta^1}\\
{\vdots}\\
{\beta^m}\end{array} \right] 
& = {} 
...
\alpha^n
  \end{array} \right],
\end{align}
with the terms $\frac{\partial f^j}{\partial x^i}$ forming the \textit{Jacobian matrix}. $F_*$ is also known as the \textit{differential} of $F$, denoted by $dF$. 

\subsubsection{The tangent velocity vector}\label{sec:tangent_velo}
The pushforward allows us to define the notion of the tangent \textit{velocity vector} to a curve. Roughly speaking, given a {curve} $F:(a,b)\rightarrow N$, the pushforward of the vector $\partial_{t, t_0} \triangleq \frac{d}{dt} \bigr|_{t_0}\in T_{t_0}(a,b)$ via $F_*$ results in the velocity vector $F_*(\partial_{t, t_0})\in T_{F(t_0)}N$; we can think of this as ``differentiating'' $F$ with respect to $t$ (at $t_0$). We describe this more formally next. 

Consider the manifolds $M = (a,b)$ and $N$, along with a map $F:M\rightarrow N$, and note that $\partial_{t,t_0}$ is a basis for $M$ at $t_0$. Let $p=F(t_0)$, and $f\in C^{\infty}(p)$, so that the {velocity vector} to the curve at $p$ is defined by 
\begin{align}\label{eq:velocityvec}
F_*\left(\partial_{t,t_0}\right)f = \partial_t\left(f\circ F\right)_{t_0}.
\end{align}
Given a chart $(U \ni p, x^1, \dots, x^n)$, so that in local coordinates $\hat F = (f^1(t), \dots, f^n(t)) \triangleq (x^1\circ F(t), \dots, x^n \circ F(t))$. 
...
% \end{aligned}
% \end{equation}
with the dot indicating the derivative with respect to $t$ (often interpreted as time). The velocity vector may then be expanded in the basis of $T_pN$ as $F_*(\partial_{t, t_0}) = \dot f^i(t_0)E_{ip}$. 
...
Because $\theta$ is an imbedding, the Jacobian $\left(\partial \theta^j / \partial x^i\right)_{u_0}$ has rank $n$ (see the definitions of immersion and imbedding in \cite{boothby_2002} for details\footnote{{\color{red}replace with ref to the imbedding / rank theorem section above if/when that becomes available.}}), and thus the vectors $ \left(X_{u^i}\right)_0$, $i=1,\dots, n$ span $T_{\theta(u_0)}N$, which is a subspace of $T_{\theta(u_0)}\R^m$. 

An alternate description of the tangent plane at a point $\theta(u_0)$ is the collection of all tangent vectors at that point to curves passing through that point which lie in $N$. To give a little more detail, we proceed as follows. 
...
\end{defn}
A vector field $X: N \rightarrow TN$ assigns a vector to each point on a manifold, such that the vector lies in the tangent space at that point, and the assignment is \textit{smooth}. Smoothness is defined as follows. 

For a chart $(U\ni p, \varphi)$ of $N$, and $\{E_{ip}\}_{i = 1}^n$ a basis of $T_pN$, a vector may be written $X_p = \alpha^i(p) E_{ip}$. If $p$ is varied in $U$, so that we consider different vectors in the vector field, the components $\alpha^i$ are real-valued functions on $N$, which we require to be smooth. In local coordinates, we require 
...
We note that this is an example of a one-parameter Lie group ($\R$) acting on a smooth manifold, where the group operation ($+$) is a smooth map, and the identity element is $0$. The \textit{orbit} of a point $p$ is the set of all points ``reachable'' from this point, $\text{orb}_{\theta}(p) \triangleq \left\{\theta_t(p) \mid t\in \R \right\}$. 
...
\end{align}
\end{defn}
\begin{thm}\label{thm:infgen_invariance}
The infinetesimal generator of an action is invariant under that action; that is, $\theta_{t*}X_p = X_{\theta_{t}(p)}$.\end{thm} 
...
&={} X_{\theta_t(p)}f, 
\end{aligned}\end{equation}
where we have used the fact that $\R$ is an Abelian group to swap the order of $\Delta t$ and $t$. \end{proof}
...
Parameterize $\text{orb}_{\theta}(p)$ using parameter $t$ by writing $F(t) = \theta_t(p)$. Consider an arbitrary point $t_0\in \R$, and the vector $\partial_{t, t_0}\triangleq \frac{d}{dt}\bigr|_{t_0}$, which is a basis of $T_{t_0}\R$. 
 From the rank theorem, we know that $F$ is an immersion of $\R$ into $N$ if and only if $F_*\left(\partial_{t, t_0}\right) \neq 0$ for all $t_0\in \R$ (i.e., the Jacobian describing $F_*$ has a constant rank (of $1$) at all points on $\R$).\footnote{{\color{red}oct 7 '24: need to write at least high level / brief notes on the rank of a mapping, immersion and imbedding. }} Now consider $f\in C^{\infty}(F(t_0))$, and by the same arguments used in the proof of Theorem \ref{thm:infgen_invariance}, we have 
 \begin{equation}\begin{aligned}\label{eq:orbit_immersion}
F_*(\partial_{t,t_0})f = \partial_{t}\left(f\circ F\right)_{t_0} &={} \lim_{\Delta t \rightarrow 0}\frac{f\circ F\left(t_0 + \Delta t\right)-f\circ F(t_0)}{\Delta t}\\
...
Now, if $X_q \neq 0$ for any $q\in \text{orb}_{\theta}(p)$, then by Corollary \ref{prop:gen_on_orbits_alts}, $X_{\theta_{t_0}(p)}$ is nonzero at all $t_0$, and $F$ is indeed an immersion. Furthermore, by the same corollary, if $X_{\theta_{t_0}(p)} = 0$, then this is true for all points on the orbit, and so $F$ is a constant map (its Jacobian having a constant rank of $0$).\footnote{revisit after doing the immersion / rank thm sections.}
\end{proof}
...
% \end{itemize}
Note that the formula just obtained, namely $F_*(\partial_{t,t_0}) = X_{F(t_0)}$ says that, given an action $\theta$ and its infinitesimal generator $X$, at each point $p$, $X_p$ is the tangent velocity vector (see Section \ref{sec:tangent_velo}) of the curve $t\rightarrow F(t)$, that is, to the curve describing the orbit generated by this action. {\color{red}We will often write $F_*(\partial_{t,t_0})$ as $\dot F(t_0)$ or $\dot \theta(t_0, p)$. Note that these, being tangent vectors, are different from the time derivatives of \textit{scalar} quantities, $\dot f^i(t)$, introduced in Section \ref{sec:tangent_velo}. }\footnote{I dont like any of the alts to $F_*(\partial_{t,t_0})$. They lose the idea that we are looking at a vector. Write the rest of the document then decide.. it might be best to use the $\dot F(t)$ alt, despite me not liking it (it doesnt have the base point). But the dot does implicate the velocity-ness wrt time.. of the displacement / flow curve F. }
...
\end{defn}
In other words, the discussion above says that the orbit of an action is the integral curve of the infinitesimal generator of that action, $\dot \theta(t,p) = X_{\theta(t,p)}$ (for a given $p$). 
% .\\
% .\\
% .\\
...
% so i actually just took it as an axiom but here are some answers that seem reasonable. (for the first one i am not sure *why* you want the bracket to satisfy the Leibniz rule with respect to the "multiplication" defined by the bracket itself). Probably some sort of closure property, analogous to the lie derivative of a C^{\infty} function staying in C^{\infty} itself, allowing for repeated lie derivatives

% probably just closure. Like you want the "application" of the lie bracket to a vector field to be like the application of a vector to a function. where for functions, mult is usual pointwise mult in R, while for VFs, the mult is the lie bracket itself
...
\subsection{Differential one-forms and codistributions}\label{sec:1FandCodistrib}
...
where the addition and multiplication on the RHS are of course operations in $\R$. 
    \item For vector spaces $V$, $W$, and duals $V^*$ and $W^*$, the linear maps $F_*:V\rightarrow W$ and $F^*:W^*\rightarrow V^*$ are related by $(F^*\sigma)(v) = \sigma (F_*(v))$. Injectivity (surjectivity) of $F_*$ implies surjectivity (injectivity) of $F^*$. %To see why, just think for 20 seconds. 
    \item A basis $\left\{E_1, \dots, E_n\right\} \subset V$ induces a basis $\left\{\omega^1, \dots, \omega^n\right\} \subset V^*$ satisfying $\omega^i(E_j) = \delta^i_j$. This implies that for a vector $v = v^i E_i$, the coefficients can be extracted using the basis covectors, as in $v^j = \omega^j(v)$. 
\end{itemize}

\subsubsection{One-forms and the differential of a function}

Given a manifold $N$, and its tangent space at $p$, $T_pN $, its dual \textit{cotangent space at $p$} is denoted by $T^*_pN$, and these can be collected via disjoint union into a cotangent bundle, denoted by $T^*N$ (c.f. Eq. \ref{eq:tangentbundle}). Here, a covector is a mapping $\sigma_p : T_pN\rightarrow \R$, and as in the case of generic vector and covector spaces (Section \ref{sec:covectors}), a basis $E_{1p}, \dots, E_{np}$ of $T_pN$ induces a covector basis $\omega^1_p, \dots, \omega^n_p$ of $T^*_pN$. A covector $\sigma_p$ can be expanded in this basis as $\sigma_p = \sigma_p(E_{ip})\omega^i_p$, where $\sigma_{ip}\triangleq\sigma_p(E_{ip})$ is the coefficient of its $i$-th component. Considering covectors as a functions of points on a manifold leads to a notion dual to vector fields, called one-forms. 
...
Note that analogously to the vector field case, a smooth one-form is a (smooth) section of the cotangent bundle, satisfying $\tilde \pi \circ \sigma = i_N$, where $\tilde \pi: T^*N \rightarrow N$ is the natural projection, and $i_N$ the identity. 
...
The field of frames $E_1, \dots, E_n$ induces (pointwise) a \textit{field of (coordinate) coframes} $\omega^1, \dots, \omega^n$ on $U$. This allows us to expand a one-form as the linear combination $\sigma = \sigma_i\omega^i$, with $\sigma_i$ as in Definition \ref{def:oneform}. Given a vector field $X$ and a one-form $\sigma$ on $U\subset N$, $\sigma(X)$ is a smooth (since $X$ and $\sigma$ are smooth) real-valued function on $U$, with $(\sigma(X))(p) = \sigma_p(X_p)$. 
 ...
 Let $f: N \rightarrow \R$ be a smooth function and $X = \alpha^i E_{i}$ a smooth vector field. This defines a one-form $df$ called the \textit{differential} of $f$ via the prescription $df(X) = X(f) \in C^{\infty}(N)$, where its value at a point is
\begin{equation}\label{eq:exact_differential_function}
 \begin{aligned}
df(X)(p) &={} X_p(f) \\&={} \alpha^{i}(p)E_{ip}(f) \\
&={} \alpha^i(p)\partial_{ip}f\qquad \text{(dropping the caret from $\hat f$)}
 \end{aligned}
 \end{equation}

On an open set $U\subset \R^n$ with coordinate functions $x^1, \dots, x^n$ and field of frames $\partial_1, \dots, \partial_n$, we can define a field of coframes $dx^1, \dots, dx^n$, which is dual in the sense that $dx^i(\partial_j) = \delta^i_j$. We may extract the $i$-th component of a vector field $X$ in the natural basis as $dx^i(X)$, and write $X = dx^i(X)\partial_i$. 

% To derive an expression for $df$ in local coordinates, that is, to find the coefficients in $df = \beta_i dx^i$, we start with Eq. \ref{eq:exact_differential_function} (over all $p$), 
% % \begin{equation}\begin{aligned}
% % df(X) = df(\alpha^i\partial_i) & = \left(\beta_i dx^i\right)\left(\alpha^j\partial_j\right) \\
% % & = \beta_i \alpha^j dx^i(\partial_j)\\
% % & = \beta_i \alpha^i = \alpha^i\partial_{i}f \qquad \text{from Eq \ref{eq:exact_differential_function},}
% % \end{aligned}
% % \end{equation}
% \begin{equation}\begin{aligned}
...
To derive an expression for $df$ in local coordinates, that is, to find the coefficients $\beta_i$ in $df = \beta_i dx^i$, note that $\partial_j f = df(\partial_j) = (\beta_i dx^i)(\partial_j) = \beta_j$
% simply apply this equation to $\partial_j$, giving 
% \begin{equation}\begin{aligned}
...
so that % $df = \left(\partial_i f \right)dx^i$. %$df = \frac{\partial f}{\partial x^i}dx^i$. 
\begin{equation}\begin{aligned}\label{eq:exact_differential_expansion}
df = & \left(\partial_i f \right)dx^i\\
= & \frac{\partial f}{\partial x^1}dx^1+\cdots + \frac{\partial f}{\partial x^n}dx^n. %f \frac{\partial f}{\partial x^i}
\end{aligned}\end{equation}
\begin{remark}
It is customary to write expansions in terms of covector bases $dx^1, \dots, dx^n$ as row vectors, while expansions in terms of vector bases $\frac{\partial}{\partial x^1}, \dots, \frac{\partial}{\partial x^n} $ as column vectors. Looking at Eqs. \ref{eq:exact_differential_function} and \ref{eq:exact_differential_expansion}, we can write
\begin{align}\label{row_and_column_vectors}
...
\end{array}\right],\\
X &= \alpha^1 \frac{\partial}{\partial x^1} +\dots + \alpha^n \frac{\partial}{\partial x^n} = \left[\begin{array}{c}
 \alpha^1\\
\vdots\\
\alpha^n
  \end{array} \right]
\end{align}
This allows us to write Eq. \ref{eq:exact_differential_function} in matrix form,  
\begin{align}\label{exact_differential_matrix}
\begin{array}{c}
df(X) \end{array}  
& = {} 
\left[\begin{array}{ccc}
...
\alpha^n
  \end{array} \right],
\end{align}
which should be compared with Eq. \ref{eq:pushforward_in_lr_matrix}.
\end{remark}


Any real-valued smooth function $f$ defines a one-form $df\in T^*N$ via Equation \ref{eq:exact_differential_function}. However, not every one form can be written as a differential $df$ of a function $f$. Those that can, are called \textit{exact}. A necessary condition for a one-form $\sigma = \sigma_i dx^i$ to be exact is that it be closed, with the defining characteristic 
\begin{align}\label{eq:closedOneForm}
\partial_j \sigma_i = \partial_i \sigma_j, \qquad \qquad \text{(closed one-form)},
\end{align}
 for all $i, j = 1, \dots, n$. This can be see by recalling the identity $\partial_i \partial_j f = \partial_j \partial_i f$. The converse is partially true, where Condition \ref{eq:closedOneForm} is sufficient for the local existence of a function $f$ such that $\sigma_i = \partial_i f$. 
...
\begin{defn}[Observation space]
    The \textit{observation space} $\mathscr{O}$ of the system \eqref{eq:system} is the linear space (over $\R$) of real valued smooth functions on $N$ containing $h_i, i = 1, \dots, p$ and all repeated Lie derivatives 
    \begin{align}
...
\section{Todos}
\begin{itemize}
	\item Initial document should only have observability and identifiability. modify the intro and title to reflect that. the one with full controllability if version 2, coming after calibration is done. 
	\item ...
\end{itemize}
