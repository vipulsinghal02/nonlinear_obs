...
\end{aligned}
\end{equation}
The basis $\left\{E_{ip}\right\}_{i=1}^n$ is called a \textit{coordinate frame} of $T_pU$, and notice that unlike in the $T_p\R^n$ case discussed previously, here we distinguish between the $E_{i(\cdot)}$ and $\frac{\partial}{\partial x^i}$ (though they are equivalent up to isomorphism, {\color{red}and will be used interchangeably in the sequel) $\leftarrow$ check this later.}. 
